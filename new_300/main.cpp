...
//  int selected = 0;;
  ...
  do{   
    ...
    //Checks the tile the player wants to move to. If it is valid, then the player moves to it and his energy is decremented
    //The tile is valid if it = 'P' or the player has purchased the item to go through the obstacle in the way
    {
      playerTile = gameMap.passTile(userCommand); //passTile does bounds check now. Returns 'Z' if player tries to move out of bounds
      if((playerTile == 'P') || ((isObstacle(playerTile) && myHero.hasItem(playerTile)))){
        moveCode = gameMap.movePlayer(userCommand); //funtion name changed from validMove to moveplayer
      ...
      //if there's an obstacle in the way and player hasn't purchased the item to go through it, error message prints and loop continues
      else if(isObstacle(playerTile))
          cout << "You can't move there! An obstacle is in your way.\n";
      //ERROR
      else
          cout << "Invalid input.\n";
    }
